...
// Transfer
const Edit = () => {
  const { tiendas, usuarios, producto , transferencia,  auth} = usePage().props;
  const { data, setData, errors, post, processing } = useForm({
    send_by: transferencia?.send_by,
    received_by: transferencia?.received_by,
    old_company: transferencia?.old_company,
    new_company: transferencia?.new_company,
    note: transferencia?.note,
  ...
  const [carrito, setCarrito] = useState(transferencia?.product_id != null ? JSON.parse(transferencia.product_id) : []);
  ...
  const checkPin = () =>{
    let user = usuarios.find(us => us.id == data.received_by)
    console.log(pin, user.pin)
    if(pin ==  user.pin){
      closeModal()
      console.log(data);
      post(route('transfer.update', transferencia?.id), ()=>{
  ...
  return (
    <div>
      <h1 className="mb-8 text-3xl font-bold">
        ...
        <span className="font-medium text-indigo-600"> /</span> Ver Transferencia
      </h1>
          ...
          {/* <div className="pl-8">
            ...
            >
             ...
             <TextInput
              className="w-full pb-8 pr-6 lg:w-1/3"
              label="Descripcion"
              disabled={data.confirmation == true}
...
Edit.layout = page => <Layout title="Ver Transferencia" children={page} />;
